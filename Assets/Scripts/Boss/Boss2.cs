...
public class Boss2 : MonoBehaviour {
	...
	void	StanceManager()
	{
		...
		if (defStance)
		{
			...
			Collider2D[] players = Physics2D.OverlapCircleAll(this.transform.position, spikesRadius);
			for (int i = 0; i < players.Length; i++)
			{
				if (players[i].gameObject.tag == "Player")
					players[i].gameObject.GetComponent<PlayerStats>().TakeDamage(0.5f);
			}
		}
	}
	...
	void OnDrawGizmosSelected()
    {
        ...
        Gizmos.DrawWireSphere(this.transform.position, spikesRadius);
